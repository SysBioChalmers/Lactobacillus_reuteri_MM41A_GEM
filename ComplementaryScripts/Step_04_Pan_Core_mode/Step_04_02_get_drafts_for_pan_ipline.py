...
def get_Lreu_from_tp(qseq_file, sseq_file, tp_model):
    ...
    blast_result_df = My_def.seq_ana.select_blast(blast_result_s_in_q, blast_result_q_in_s, best_match=True,
...
Lreuteri_summery = pd.read_csv('../Initial_data/other_Lreuteri_seq/Lreuteri_summery.csv', sep='\t', header=0,
                               dtype='str')
Lreuteri_summery_selected = Lreuteri_summery[Lreuteri_summery['seq_files'] == 'True']
Lreuteri_summery_selected = Lreuteri_summery_selected[Lreuteri_summery_selected['genome.genome_status'] != 'Plasmid']
Lreuteri_summery_selected = Lreuteri_summery_selected.drop_duplicates(subset='Strain_name')
Lreuteri_summery_selected.to_csv('Lreuteri_summery_selected.csv', sep='\t', index=False)

for id, strain in zip(Lreuteri_summery_selected['genome.genome_id'], Lreuteri_summery_selected['Strain_name']):
    faa_name = id + '.PATRIC.faa'
    os.system(
        'cp ../Initial_data/other_Lreuteri_seq/seqs_from_PATRIC/' + faa_name + ' seqs_from_PATRIC/' + strain + '.faa')

Lreuteri_summery_selected = pd.read_csv('Lreuteri_summery_selected.csv', sep='\t', header=0, dtype='str')
name_list = Lreuteri_summery_selected['Strain_name'].tolist()
...
for sp_name in name_list:
    ...
    Lreu_seq = '../other_Lreuteri_seq/' + sp_name + '.faa'
    ...
    os.chdir('../other_Lreuteri_moldebuilding/')
    ...
    solution_biomass_f = cobra.flux_analysis.gapfilling.GapFiller(Lreu_draft_3, iNF517, demand_reactions=False,
    ...
    Lreu_draft_3.objective = "BIOMASS_LRE"
    print('Lreu_draft_3 Biomass:', Lreu_draft_3.optimize())
    Lreu_draft_3.id = sp_name
    cobra.io.save_json_model(Lreu_draft_3, sp_name + 'Lreu_draft_3.json', sort='True')
